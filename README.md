...
> Answer: The main benefit to client-side-routing is the potential load time between views is faster and the perceived load times of interactive elements is faster for the user. Client side routing also provides developers and designers with more flexibility implementing transitions and animations between views. With client side routing, the application only needs to load the newly requested data, so the application doesn't need to request components like the navbar and the footer on every view change.
